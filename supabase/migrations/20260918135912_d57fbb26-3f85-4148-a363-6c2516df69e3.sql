...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_periodos) LOOP
    ...
    IF v_inicio < time '08:00' OR v_fim > time '18:00' OR v_inicio >= v_fim THEN RAISE EXCEPTION 'Escolha horários válidos entre 08:00 e 18:00.'; END IF;
...
CREATE OR REPLACE FUNCTION public.submit_public_consultation(p_nome text, p_email text, p_whatsapp text, p_tipo_negocio text)
...
BEGIN
  ...
  IF length(trim(coalesce(p_whatsapp, ''))) > 30 OR length(regexp_replace(coalesce(p_whatsapp, ''), '\D', '', 'g')) < 10 THEN RAISE EXCEPTION 'Informe um WhatsApp válido.'; END IF;
  IF length(trim(coalesce(p_tipo_negocio, ''))) < 2 OR length(trim(p_tipo_negocio)) > 120 THEN RAISE EXCEPTION 'Informe um tipo de negócio válido de até 120 caracteres.'; END IF;
  INSERT INTO public.contract_requests (user_id, nome, email, telefone, ambiente, plano_tipo, preco, dias_selecionados, observacoes, status, nicho, origem)
  VALUES (auth.uid(), trim(p_nome), lower(trim(p_email)), trim(p_whatsapp), 'estacao'::public.ambiente_tipo, 'hora', 0, '[]'::jsonb, 'Consulta de disponibilidade pelo site.', 'pendente', trim(p_tipo_negocio), 'Consulta pelo site') RETURNING id INTO v_id;
  RETURN v_id;
END;
$function$;