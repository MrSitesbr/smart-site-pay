...
import { ChevronLeft, ChevronRight, ExternalLink, Trash2, DollarSign, Pencil, X } from "lucide-react";
...
export default function DayTimelinePanel({ day, onChangeDay, onClose, reservas, contratos, gEvents, openHour = 9, closeHour = 17, onDeleteReserva, onDeleteContrato, onEditReserva }: Props) {
  ...
  const bars = useMemo<Bar[]>(() => {
    ...
    return out;
  }, [day, reservas, contratos, gEvents, internalIds, OPEN_HOUR, CLOSE_HOUR]);
  ...
  return (
    <div className="border-t">
      <div className="flex items-center justify-between gap-2 p-3 border-b bg-muted/20">
        ...
        <h3 className="font-heading font-bold text-center flex-1 capitalize text-base">
        ...
        <div className="flex items-center gap-1">
          ...
          {onClose && (
            <Button size="icon" variant="ghost" title="Fechar linha do tempo" onClick={onClose}>
              <X className="w-5 h-5" />
            </Button>
          )}
        </div>
            ...
            return (
              <>
                ...
                <div className="flex flex-wrap justify-end gap-2 mt-2">
                  ...
                  {b.kind === "reserva" && onEditReserva && <Button variant="outline" size="sm" onClick={() => { onEditReserva(b.obj); setSelected(null); }}><Pencil className="w-4 h-4 mr-2" /> Editar</Button>}
                  {!isGoogle && (
                    <Button
