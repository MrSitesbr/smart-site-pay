...
import { Trash2, Settings2, AlignLeft, AlignCenter, AlignRight, X, Download } from "lucide-react";
export const Inspector = ({ type, data, onUpdate, onClose, onDelete }) => {
    ...
    return (<div className="flex flex-col h-full bg-white border-l shadow-xl w-[400px] z-50 animate-in slide-in-from-right duration-300">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="p-6">
            <TabsContent value="content" className="mt-0 space-y-6">
              {type === 'widget' && (<>
                  {data.type === 'heading' || data.type === 'text' || data.type === 'button' ? (<div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Texto</Label>
                      <Textarea value={data.content.text || ''} onChange={(e) => handleChange('content.text', e.target.value)} className="min-h-[100px] text-sm"/>
                    </div>) : null}

                  {data.type === 'heading' && (<div className="space-y-2">
                      ...
                      <Select value={data.content.level || 'h2'} onValueChange={(v) => handleChange('content.level', v)}>
                        ...
                        <SelectContent>
                          {['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p'].map(lvl => (<SelectItem key={lvl} value={lvl}>{lvl.toUpperCase()}</SelectItem>))}
                        </SelectContent>
                      </Select>
                    </div>)}

                  {data.type === 'image' && (<div className="space-y-2">
                  ...
                  {data.type === 'form' && (<div className="space-y-2">
              ...
              {type === 'column' && (<div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Largura (%)</Label>
                    <Input type="number" value={data.widthPercentage || 100} onChange={(e) => handleChange('widthPercentage', parseInt(e.target.value))}/>
                  </div>
                </div>)}

              {type === 'section' && (<div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                    <Label className="text-[10px] font-black uppercase tracking-widest">Largura Total</Label>
                    <input type="checkbox" checked={data.settings.fullWidth || false} onChange={(e) => handleChange('settings.fullWidth', e.target.checked)} className="w-4 h-4 accent-brand-orange"/>
                  </div>
                </div>)}
            </TabsContent>
            ...
            <TabsContent value="style" className="mt-0 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input type="color" className="w-10 h-10 p-1" value={data.styles?.color || data.settings?.backgroundColor || "#000000"} onChange={(e) => type === 'widget' ? handleChange('styles.color', e.target.value) : handleChange('settings.backgroundColor', e.target.value)}/>
                  </div>
                </div>

                {type === 'widget' && (<div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Alinhamento</Label>
                    <div className="flex bg-muted p-1 rounded-lg">
                      <Button variant={data.styles?.alignment === 'left' ? 'secondary' : 'ghost'} size="icon" className="h-8 flex-1" onClick={() => handleChange('styles.alignment', 'left')}>
                        <AlignLeft className="w-4 h-4"/>
                      </Button>
                      <Button variant={data.styles?.alignment === 'center' ? 'secondary' : 'ghost'} size="icon" className="h-8 flex-1" onClick={() => handleChange('styles.alignment', 'center')}>
                        <AlignCenter className="w-4 h-4"/>
                      </Button>
                      <Button variant={data.styles?.alignment === 'right' ? 'secondary' : 'ghost'} size="icon" className="h-8 flex-1" onClick={() => handleChange('styles.alignment', 'right')}>
              ...
              {type === 'section' && (<div className="space-y-2">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    ...
                    <Input type="number" value={data.settings?.padding?.top || data.styles?.padding?.top || 0} onChange={(e) => type === 'widget' ? handleChange('styles.padding.top', parseInt(e.target.value)) : handleChange('settings.padding.top', parseInt(e.target.value))}/>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[8px] font-bold">BOTTOM</Label>
                    <Input type="number" value={data.settings?.padding?.bottom || data.styles?.padding?.bottom || 0} onChange={(e) => type === 'widget' ? handleChange('styles.padding.bottom', parseInt(e.target.value)) : handleChange('settings.padding.bottom', parseInt(e.target.value))}/>
                  </div>
                </div>
              </div>
      ...
      <div className="p-4 border-t bg-muted/20 flex flex-col gap-2">
        {type === 'section' && (<Button variant="outline" className="w-full font-bold border-brand-blue-dark text-brand-blue-dark hover:bg-brand-blue-dark hover:text-white" onClick={() => {
        ...
        {onDelete && (<Button variant="destructive" className="w-full font-bold" onClick={onDelete}>
