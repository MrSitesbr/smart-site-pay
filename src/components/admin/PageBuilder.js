...
import { Plus, Save, Layout, Eye, Smartphone, Monitor, Redo, Undo, Search, Download, Upload } from "lucide-react";
import { PageRenderer } from "@/components/PageRenderer";
import { Inspector } from "./Inspector";
import { WIDGET_REGISTRY } from "./WidgetRegistry";
import { toast } from "sonner";
export const PageBuilder = ({ pageId, initialLayout = [], onSave }) => {
    ...
    const addSection = (columnCount = 1) => {
        const newSection = {
    ...
    return (<div className="flex h-screen bg-[#f1f1f1] overflow-hidden font-sans">
      ...
      <div className="w-[300px] bg-white border-r flex flex-col shadow-xl z-30">
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(WIDGET_REGISTRY).map(([type, config]) => (<div key={type} className="flex flex-col items-center justify-center p-4 bg-brand-gray/20 rounded-xl border border-transparent hover:border-brand-orange hover:bg-white transition-all cursor-move group" draggable onDragEnd={() => {
        ...
        <div className="p-4 border-t bg-muted/20">
          <Button className="w-full bg-brand-blue-dark hover:bg-brand-blue-dark/90 text-white font-bold py-6" onClick={() => addSection(1)}>
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            <div className="flex gap-2 mr-2">
              ...
              <div className="relative">
                <Button variant="outline" size="sm" className="font-bold text-[10px] h-8" onClick={() => document.getElementById('import-json-input')?.click()}>
                  <Upload className="w-3 h-3 mr-1"/> IMPORTAR
                </Button>
                <input id="import-json-input" type="file" accept=".json" className="hidden" onChange={handleImportJSON}/>
              </div>
            </div>
            ...
            <Button className="bg-brand-orange hover:bg-brand-orange/90 text-white font-bold shadow-lg shadow-brand-orange/20" onClick={handleSave} disabled={isSaving}>
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar p-12 transition-all duration-500 ${viewMode === 'mobile' ? 'max-w-[400px] mx-auto shadow-2xl border-x-8 border-t-8 border-brand-blue-dark rounded-t-3xl mt-4 bg-white' : 'bg-[#f1f1f1]'}`}>
          <div className={`${viewMode === 'desktop' ? 'bg-white shadow-2xl min-h-full rounded-xl overflow-hidden' : ''}`}>
            {layout.length === 0 ? (<div className="flex flex-col items-center justify-center py-40 border-4 border-dashed border-muted/50 rounded-xl m-8">
              ...
              </div>) : (<PageRenderer layout={layout} isAdmin={true} onElementClick={handleElementClick}/>)}
            
            {layout.length > 0 && (<div className="p-12 flex justify-center border-t border-dashed mt-12 bg-muted/5">
                <Button onClick={() => addSection(1)} variant="outline" className="border-2 border-dashed border-brand-orange text-brand-orange hover:bg-brand-orange hover:text-white font-black py-8 px-12 rounded-2xl transition-all">
      ...
      {selectedElement && (<Inspector type={selectedElement.type} data={selectedElement.data} onUpdate={updateElement} onClose={() => setSelectedElement(null)} onDelete={deleteElement}/>)}
    </div>);
};
