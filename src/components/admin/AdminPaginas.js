...
import { FileText, Plus, Eye, Layout, ChevronRight, ChevronLeft } from "lucide-react";
import { getPageContent, updateSectionContent } from "@/lib/cms";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { PageBuilder } from "./PageBuilder";
export default function AdminPaginas({ mode = 'pages' }) {
    ...
    const handleSavePage = async (layout) => {
        if (!selectedPage)
            return;
        try {
            const dynamicSection = selectedPage.site_sections?.find((s) => s.section_key === 'dynamic-layout') || selectedPage.site_sections?.[0];
            if (dynamicSection) {
    ...
    if (isBuilding && selectedPage) {
        ...
        <PageBuilder pageId={selectedPage.id} initialLayout={initialLayout} onSave={handleSavePage}/>
        <Button variant="ghost" size="sm" className="absolute top-3 left-4 z-[60] text-white hover:bg-white/10" onClick={() => setIsBuilding(false)}>
    ...
    if (selectedPage) {
        ...
        <div className="flex items-center justify-between sticky top-0 bg-background/80 backdrop-blur-md z-10 py-4">
          ...
          <div className="flex items-center gap-2">
            <Button className="bg-brand-orange hover:bg-brand-orange/90 text-white font-bold" onClick={() => setIsBuilding(true)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {selectedPage.site_sections?.sort((a, b) => a.order_index - b.order_index).map((section) => (<Card key={section.id} className="p-6 border-none shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
          ...
          <Button className="w-full border-dashed border-2 py-12 bg-transparent text-muted-foreground hover:bg-brand-orange/5 hover:text-brand-orange hover:border-brand-orange transition-all rounded-3xl flex flex-col gap-2" onClick={() => setIsBuilding(true)}>
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPages.map(p => (<Card key={p.id} className={`group p-8 cursor-pointer border-none shadow-sm hover:shadow-xl transition-all relative overflow-hidden bg-white ${p.is_global ? 'border-l-4 border-l-brand-orange' : ''}`} onClick={() => loadPage(p.route)}>
